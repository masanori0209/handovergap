...
| Result models | `transferability_status`, `transferability_score`, `gaps`, `questions`, `scenario_id`, `profile`, `memory`, `task_context`, `required_slots`, `provided_slots`, `evidence_slots`, `high_risk_slots`, gap fields, question fields, follow-up retrieval query fields, and product route fields such as `action`, `recommended_action`, `deployment_mode`, `retrieval_mode`, `safety_policy`, `enforcement`, `should_interrupt`, and `next_step`. |
