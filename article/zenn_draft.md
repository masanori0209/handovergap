...
OpenAIによるsemantic slot fillingとTiDBへの監査保存まで画面から試す場合:
...
Live modeでは、架空の引き継ぎケースに対して次の処理を行います。

1. retrieved memoryと関連証拠を表示
2. OpenAI modelでrole-required slotをsemantic slot filling
3. HandoverGap detectorで不足slot、確認質問、transferabilityを判定
4. slot_fill_attempts、context_gaps、transfer_assessmentsをTiDBへ保存

このLive demoは `gpt-5-mini` とTiDB Cloud Developer Tierで疎通確認済みです。最新のPyPI版では `handovergap==0.1.3` として公開しています。
...
- StreamlitのLive modeと検証スクリプトでは、OpenAIによる意味的slot fillingを実行できます。
- LLMがslotを控えめ/楽観的に埋めた場合の揺れはholdout stress profileで模擬し、任意のOpenAI実接続slot fillingでも検証しています。
- Question Coverageはslot一致で評価し、意味的同値判定は行いません。
- 組織ごとに役割要件と重要度の調整が必要です。
- 実データではプライバシー、アクセス制御、保持期間の設計が必要です。
- ライブTiDBへのschema作成、評価結果保存、Live demoからのslot/gap/transfer監査保存は検証済みですが、負荷・障害試験は今後の課題です。
