...
回答許可の厳しさは safety policy で切り替えられます。

| safety policy | 挙動 |
| --- | --- |
| `strict` | デフォルト。profileで `high_risk` なslotは、明示的な `evidence_slots` がない限り回答に戻さない |
| `balanced` | profile必須slotが埋まっていれば、呼び出し側が渡した `provided_slots` を信頼する |
| `exploratory` | 観測・実験向け。同じAPI面を使いつつ、アプリ側で強制しない用途に使う |
...
print(route.safety_policy)
print(route.next_step)
print(route.retrieval_queries)
print(route.questions)
```

`strict` では、authority、顧客への説明状況、代替手段、エスカレーション先、契約影響のような高リスクslotは、追加検索後も証拠で支えられていない限り `answer` に戻しません。slot fillerを十分にレビューできている場合だけ `balanced` に切り替える想定です。
...
## ユーザーデータでのannotation / evaluation

同梱datasetは再現性のための架空データです。本番導入前には、自分たちの匿名化済みデータをローカルで `annotate -> labels import -> evaluate -> report` する必要があります。

匿名化済みのJSONL、JSON、CSVに `scenario_id`、`memory`、`profile`、`memory_type`、`task_context`、`provided_slots`、`evidence_slots` を用意し、annotation templateを出力します。

```bash
handovergap datasets export-template ./local/anonymized_scenarios.jsonl --output ./local/labels.csv
```

templateにはscenario id、profile、task context、required slots、slot列が入りますが、raw memoryやevidence本文はコピーしません。レビュアーはローカルで `gold_gap_slots`、`gold_question_slots`、`unsafe_transfer_label`、`annotation_notes` を埋めます。

レビュー済みlabelをmergeして評価します。
